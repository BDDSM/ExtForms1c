...
Процедура ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, МетаМетка, РасчетОбъема, ИменаСУБД)
	...
	ТаблицаРезультата.Колонки.Добавить("ИмяТаблицыХранения");
	ТаблицаРезультата.Колонки.Добавить("Назначение");
	ТаблицаРезультата.Колонки.Добавить("Объем", тЧисло(19, 2));
	//ПоляСорт = ПолучитьПоляСортировки();
	...
	Для Каждого МетаОбъект Из Метаданные[ТипМетаданных] Цикл
		...
		Для Каждого СтрокаОбъект Из СтруктураБД Цикл
			...
			НоваяСтрока.Объем = ПолучитьОбъемТаблицыСУБД(НоваяСтрока.ИмяТаблицыХранения, РасчетОбъема);
...
Функция СформироватьМакетТаблицыОтчета(пПериод) Экспорт
	...
	Иначе
		...
		//МассивМетаданных = Новый Массив;
		//МассивМетаданных.Добавить(Метаданные.Документы.ПриемНаРаботуСписком);
		//
		//СтруктураБД = ПолучитьСтруктуруХраненияБазыДанных(МассивМетаданных, СтруктураВТерминахСУБД);
		//
		//ТабДок.Вывести(ОблШапка);
		//Для Каждого СтрокаОбъект Из СтруктураБД Цикл
		//	
		//	//Если Найти(СтрокаОбъект.Метаданные, "Справочник.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "Документ.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "Константа.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "Перечисление.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "ЖурналДокументов.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "ПланВидовХарактеристик.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "ПланВидовРасчета.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "ПланОбмена.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "Задача.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "БизнесПроцесс.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "РегистрРасчета.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "РегламентноеЗадание.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "РегистрНакопления.") = 1
		//	//	ИЛИ Найти(СтрокаОбъект.Метаданные, "РегистрСведений.") = 1 Тогда
		//	//	Продолжить;
		//	//КонецЕсли;
		//	
