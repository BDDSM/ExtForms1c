...
Процедура ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, МетаМетка, РасчетОбъема, ИменаСУБД, ТаблицаДанныхБД)
	...
	Для Каждого МетаОбъект Из Метаданные[ТипМетаданных] Цикл
		...
		Для Каждого СтрокаОбъект Из СтруктураБД Цикл
			...
			Если РасчетОбъема Тогда
				НайденнаяСтрока = ТаблицаДанныхБД.Найти(ПолучитьИмяТаблицыСУБД(НоваяСтрока.ИмяТаблицыХранения, ИменаСУБД), "ИмяТаблицы");
				Если НайденнаяСтрока <> Неопределено Тогда
					НоваяСтрока.КоличествоД = НайденнаяСтрока.ОбъемДаных;
					НоваяСтрока.КоличествоИ = НайденнаяСтрока.ОбъемИндекса;
					НоваяСтрока.Количество  = НайденнаяСтрока.ОбъемДаных + НайденнаяСтрока.ОбъемИндекса;
				КонецЕсли;
			КонецЕсли;
...
Функция СформироватьМакетТаблицыОтчета(пПериод) Экспорт
	...
	Иначе
		...
		ТаблицаДанныхБД = Неопределено;
		Если РасчетОбъема Тогда
			Области.Шапка.Параметры.ЕдИзм = "Кб";
			ТаблицаДанныхБД = ПолучитьТаблицуДанныхБД(РасчетОбъема);
		КонецЕсли;
		ТабДок.Вывести(Области.Шапка);
		
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "ПО",  РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "Спр", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "Док", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "ПВХ", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "ПС",  РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "ПВР", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "РегС", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "РегН", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "РегБ", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "РегР", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "БП",  РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
		ВывестиРазделОтчетаСтруктура(ТабДок, Макет, Области, "Зад", РасчетОбъема, СтруктураВТерминахСУБД, ТаблицаДанныхБД);
...
Функция ПолучитьТаблицуДанныхБД(РасчетОбъема)
	
	ТаблицыБазыДанных = Новый ТаблицаЗначений;
	ТаблицыБазыДанных.Колонки.Добавить("ИмяТаблицы"  , тСтрока(150));
	ТаблицыБазыДанных.Колонки.Добавить("ОбъемДаных"  , тЧисло(19, 2));
	ТаблицыБазыДанных.Колонки.Добавить("ОбъемИндекса", тЧисло(19, 2));
	...
	Пока RecordSet.EOF() = 0 Цикл
		...
		СтрокаТаблицыБазыДанных.ИмяТаблицы   = RecordSet.Fields("name").Value;
		СтрокаТаблицыБазыДанных.ОбъемДаных   = Число(СокрЛП(RecordSet.Fields("data").Value));
		СтрокаТаблицыБазыДанных.ОбъемИндекса = Число(СокрЛП(RecordSet.Fields("index_size").Value));
		RecordSet.MoveNext();
	КонецЦикла;
